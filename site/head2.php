<?php
    ...
    if(isset($_GET['cod1'])){
        $COD1 = $_GET['cod1'];
        $QUERY_COD1 = $mysqli->query("SELECT * FROM tbl_encabezado_1 WHERE cod1='$COD1'");
        while($row = $QUERY_COD1->fetch_assoc()) 
		{
		$TITULO_COD1 = $row['descripcion'];
		}
    } else {
        echo"<script>window.location.href='head1.php'; </script>";
    } 
	
    $LISTA_ENCABEZADO_2 = $mysqli->query("SELECT * FROM tbl_encabezado_2 WHERE cod1 ='$COD1' ORDER BY cod1");
    ...
    <body>
        <div class="container">
            ...
            <div class="row" align="center">
                <div class='alert alert-success alert-dismissible fade show' role='alert'><strong><?php echo $TITULO_COD1; ?></strong></div>
                <?php 
					// Comprobando si el servicio seleccionado tiene subnivel
					if(isset($_POST['head2']))
					{
                        $COD2 = $_POST['cboEncabezado2'];
                        $QUERY_COD2 = $mysqli->query("SELECT * FROM tbl_encabezado_2 WHERE cod1='$COD1' AND cod2='$COD2'");
            			while($row = $QUERY_COD2->fetch_assoc()) 
			            {
				           $SUBNIVEL = $row['subnivel'];
			            }
                        if ($SUBNIVEL == "S"){
                            echo"<script>window.location.href='head3.php?cod1=$COD1&cod2=$COD2';</script>";
                        } else {
                            echo"<script>window.location.href='result.php?cod1=$COD1&cod2=$COD2';</script>";
                        }
					}
				?>
                ...
                <form name="frmSelect1" method="post" action="">
                    ...
                    <div class="col-md-8" align="center">
                        ...
                        <select name="cboEncabezado2" class="form-select form-select-lg" id="responsive_text" aria-label=".form-select-lg" required>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6" align="right">
                                <a class='class="w-25 btn btn-lg btn-warning' href='head1.php' role='button'>&nbsp;&nbsp;&nbsp;Reiniciar proceso&nbsp;&nbsp;&nbsp;</a>&nbsp;&nbsp;<button class="btn btn-lg btn-primary" type="submit" name="head2">&nbsp;&nbsp;&nbsp;Siguiente&nbsp;&nbsp;&nbsp;</button>
                            </div>
                        </div>
                    </div>
